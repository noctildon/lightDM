...
def primakoff_production_diffxs(theta, energy, z, ma, g=1):
    ...
    return kAlpha * (g * z * ff * pa**2 / t)**2 * sin(theta)**3 / 4
...
def compton_production_dSdEa(ea, eg, g, ma):
    ...
    xmin = ((s - me**2)*(s - me**2 + ma**2) 
            - (s - me**2)*sqrt((s - me**2 + ma**2)**2 - 4*s*ma**2))/(2*s*(s-me**2))
    xmax = ((s - me**2)*(s - me**2 + ma**2) 
            + (s - me**2)*sqrt((s - me**2 + ma**2)**2 - 4*s*ma**2))/(2*s*(s-me**2))

    thresh = heaviside(s > (me + ma)**2, 0.0)*heaviside(x-xmin,0.0)*heaviside(xmax-x,0.0)
    return thresh * (1 / eg) * pi * a * aa / (s - me ** 2) * (x / (1 - x) * (-2 * ma ** 2 / (s - me ** 2) ** 2
                                                                * (s - me ** 2 / (1 - x) - ma ** 2 / x) + x))
...
def axioelectric_xs(pe_xs, energy, z, a, g, ma):
    ...
    return 137 * 3 * g**2 * pe * energy**2 * (1 - np.power(beta, 2/3)/3) / (16*pi*me**2 * beta)
...
class PrimakoffAxionFromBeam:
    def __init__(self, photon_rates=[1.,1.,0.], target_z=90, target_photon_cross=15e-24,
        ...
        self.thetas = exp(np.random.uniform(-18, np.log(pi), nsamples))
    ...
    def simulate_kinematics_single(self, photon):
        ...
        weight *= 2*pi*(log(pi/exp(-12))/self.nsamples)
        ...
        return np.histogram2d(e_gamma*self.support, thetas_z,
                              weights=weight*convolution(self.thetas, self.phis)*self.thetas,
...
class ComptonAxionFromBeam:
    ...
    def lifetime(self):
        if 1 < 4 * (me / self.axion_mass) ** 2:
            return np.inf
        return (8 * pi) / (self.axion_coupling ** 2 * self.axion_mass * np.power(1 - 4 * (me / self.axion_mass) ** 2, 1 / 2))
    ...
    def propagate(self):  # propagate to detector
        ...
        # Get decay and survival probabilities
        surv_prob = np.array([mp.exp(-self.det_dist / meter_by_mev / v_a[i] / (axion_boost[i] * tau)) \
                     for i in range(len(v_a))])
        decay_prob = np.array([fsub(1,mp.exp(-self.det_length / meter_by_mev / v_a[i] / (axion_boost[i] * tau))) \
                      for i in range(len(v_a))])
        # TODO: remove g**2 multiplication here (was ad hoc to speed up / modularize)
    ...
    def scatter_events(self, detector_number, detector_z, detection_time, threshold):
        ...
        for i in range(len(self.axion_energy)):
            if self.axion_energy[i] >= threshold:
                xs = compton_scattering_xs(self.axion_energy[i], self.axion_coupling)
                self.scatter_weight[i] *= xs * meter_by_mev**2 * detection_time * detector_number * detector_z
...
class IsotropicAxionFromCompton:
    ...
    def AxionDecayProb(self, ea):
        ...
        return mp.exp(-self.detector_distance / meter_by_mev / axion_v / tau) \
               * fsub(1,mp.exp(-self.detector_length / meter_by_mev / axion_v / tau))
